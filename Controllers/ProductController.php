...
class ProductController
{
    ...
    {
        $conn = db();
        $sku = mysqli_escape_string($conn, $funiture->getSKU());
        $name = mysqli_escape_string($conn, $funiture->getName());
        $price = mysqli_escape_string($conn, $furniture->getPrice());
        $type = mysqli_escape_string($conn, $furniture->getType());
        $height = mysqli_escape_string($conn, $funiture->getHeight());
        $width = mysqli_escape_string($conn, $furniture->getWidth());
        $length = mysqli_escape_string($conn, $furniture->getLength());

        $sql = "INSERT INTO
       product(SKU,name,price,type,height,width,length)
       VALUES ('$sku','$name','$price',0,'$type','$height','$width','$length')";
    ...
    public function addBook($book)
    {
        ...
        $sql = "INSERT INTO
       product(SKU,name,price,type,weight)
